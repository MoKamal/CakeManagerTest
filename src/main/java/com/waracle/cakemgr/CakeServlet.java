...
import java.io.*;//todo: should be refactored to only import used classes
import java.net.URL;
import java.util.List;
import org.codehaus.jackson.map.ObjectMapper;

/**
 * Cake servlet controller class
 * 
 * @author DELL 5559
 */
@WebServlet("/cakes")
public class CakeServlet extends HttpServlet {
    ...
    private void processJsonRequest(HttpServletRequest req, HttpServletResponse response) {
        ...
        try {
            ...
            try {
                mapper.writeValue(out, new CakeManagerException(ex.getMessage()));
    ...
    private void processFormRequest(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            ex.printStackTrace();
            throw new CakeManagerException("Form Request Error: " + ex.getMessage());
        }
    }
    ...
    private void add(CakeEntity object) {
        ...
        try {
            ...
            ex.printStackTrace();
            throw new CakeManagerException("Add Error: " + ex.getMessage());
        } finally {
            HibernateUtil.close(session);
        }
    }
    ...
    private void processBrowserRequest(HttpServletRequest req, HttpServletResponse resp) {
        ...
        try {
            ...
            req.getRequestDispatcher("list.jsp").forward(req, resp);//todo:this should be refactored to seperate 
            
        } catch (Throwable ex) {
            ex.printStackTrace();
            throw new CakeManagerException("Browser Request Error: " + ex.getMessage());
        }
    }
